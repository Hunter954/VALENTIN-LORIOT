...
with app.app_context():
    ...
    if not user:
        user = AdminUser(username=ADMIN_USER, email=ADMIN_EMAIL)
        user.set_password(ADMIN_PASS)
        db.session.add(user)
    else:
        # garante que email/senha fiquem como ENV (opcional, mas ajuda)
        user.email = ADMIN_EMAIL
        user.set_password(ADMIN_PASS)

    db.session.commit()
    print("OK: banco inicializado e admin criado/atualizado.")
